// src/Offices/Assessor/Overview.jsx
import { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import {
  FileText, ArrowUpRight, Sparkles, Clock3, CheckCircle2, AlertCircle, RefreshCw, Building, ShieldCheck,
} from "lucide-react";
import OfficeLayout from "../OfficeLayout";
import { assessorOffice } from "../officeData";
import { getAssessorRequests } from "../../services/assessorData";
...
export default function AssessorOverview() {
  ...
  useEffect(() => {
    document.title = "City Assessor's Office Overview — ACORS";
    loadData();
  }, []);

  const stats = {
    total: requests.length,
    taxDec: requests.filter((r) => r.certificateType?.includes("Tax Declaration")).length,
    assessmentCert: requests.filter((r) => r.certificateType?.includes("Property Assessment")).length,
    assessedValue: requests.filter((r) => r.certificateType?.includes("Assessed Value")).length,
  ...
  const statCards = [
    { title: "Total Requests", value: stats.total, note: "All real property filings" },
    { title: "Tax Declarations", value: stats.taxDec, note: "Certified true copies" },
    { title: "Pending Review", value: stats.pending, note: "Cadastral check queue" },
    { title: "AI Passed & OCR", value: stats.aiPassed, note: "Verified PIN & TD" },
    { title: "Approved & Issued", value: stats.approved, note: "Sealed certificates" },
  ];
  ...
  return (
    <OfficeLayout office={assessorOffice} header="City Assessor's Office">
        ...
        <header className="flex animate-fade-up flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.22em] text-gray-500">
              ...
              City Government of Malaybalay · Real Property Appraisal &amp; Assessment Desk
            </p>
        ...
        <section className="grid gap-6 xl:grid-cols-12">
          <div className="animate-fade-up rounded-3xl border border-gray-200/70 bg-white p-5 shadow-sm xl:col-span-7" style={{ animationDelay: "80ms" }}>
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-xs text-gray-400">Real Property Filings</span>
            </div>
            <div className="mt-5 space-y-4">
              {assessorOffice.categoryBreakdown.map((item) => (
            ...
            <div className="mt-6 rounded-2xl bg-zinc-50 p-4 text-xs text-gray-600">
              <div className="flex items-center gap-2 font-bold text-gray-900">
                ...
                <span>AI OCR &amp; Cadastral Pre-Screening Active</span>
              </div>
              <p className="mt-1 text-[11px] text-gray-500">
                Simulated OCR automatically reads owner names, property PINs, lot numbers, and Tax Declaration numbers. Final cadastral assessment and official certification are approved by authorized Assessor staff.
              </p>
            </div>
          </div>
            ...
            <div className="mt-5 space-y-4 text-xs">
              <div className="flex items-center justify-between rounded-2xl border border-emerald-100 bg-emerald-50/50 p-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div><span className="block font-bold text-gray-900">Approved &amp; Issued</span><span className="text-[11px] text-gray-500">Certified copies &amp; certificates</span></div>
                </div>
              ...
              <div className="flex items-center justify-between rounded-2xl border border-blue-100 bg-blue-50/50 p-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div><span className="block font-bold text-gray-900">Pending Verification</span><span className="text-[11px] text-gray-500">Awaiting Assessor review</span></div>
                </div>
              ...
              <div className="flex items-center justify-between rounded-2xl border border-amber-100 bg-amber-50/50 p-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div><span className="block font-bold text-gray-900">Requires Correction</span><span className="text-[11px] text-gray-500">Missing proof of title</span></div>
                </div>
                ...
                {requests.slice(0, 6).map((req) => {
                  ...
                  return (
                    <tr key={req.id} className="transition hover:bg-gray-50/70">
                      ...
                      <td className="px-3.5 py-3.5 font-bold text-gray-900">{req.property?.ownerName || req.requester?.fullName}</td>
                      <td className="px-3.5 py-3.5 font-mono text-gray-600">{req.property?.propertyId}</td>
                      <td className="px-3.5 py-3.5">
