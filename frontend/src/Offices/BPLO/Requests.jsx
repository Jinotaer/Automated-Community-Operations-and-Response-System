// src/Offices/BPLO/Requests.jsx
import { useState, useEffect } from "react";
import {
  FileText, Search, CheckCircle2, AlertCircle, Clock3, XCircle,
  Eye, X, Sparkles, User, CreditCard, Check, ShieldCheck,
  RefreshCw, Edit3, Briefcase, Building2, Wallet,
} from "lucide-react";
import OfficeLayout from "../OfficeLayout";
import { bploOffice } from "../officeData";
import { getBPLORequests, updateBPLORequestStatus } from "../../services/bploData";

export default function BPLORequests() {
  ...
  useEffect(() => {
    document.title = "BPLO Applications & Verification — ACORS";
    loadData();
  }, []);
  ...
  const filteredRequests = requests.filter((req) => {
    const ownerName = req.owner?.fullName || "";
    const bName = req.business?.businessName || "";
    const reqId = req.id || "";
    ...
    const matchesType =
      typeFilter === "All" ||
      (typeFilter === "Renewal" && req.certificateType?.includes("Renewal")) ||
      (typeFilter === "Closure" && req.certificateType?.includes("Closure")) ||
      (typeFilter === "Retirement" && req.certificateType?.includes("Retirement"));
  ...
  function handleApprove(reqId) {
    const isRenewal = selectedRequest?.certificateType?.includes("Renewal");
    const isClosure = selectedRequest?.certificateType?.includes("Closure");
    const docNo = isRenewal
      ? `BP-2026-${Math.floor(10000 + Math.random() * 90000)}-R`
      : isClosure
      ? `BC-2026-${Math.floor(100000 + Math.random() * 900000)}`
      : `BR-2026-${Math.floor(100000 + Math.random() * 900000)}`;

    const updated = updateBPLORequestStatus(reqId, "Approved", {
    ...
    setActionFeedback({ type: "success", message: `Application ${reqId} approved & permit/certificate issued.` });
    setTimeout(() => setActionFeedback(null), 4000);
  }

  function handleReject(reqId) {
    const updated = updateBPLORequestStatus(reqId, "Rejected", {
      verificationStatus: "Rejected by BPLO Licensing Officer",
      rejectionReason: "Inconsistent zoning or outstanding local regulatory deficiencies.",
    });
    setRequests(updated);
    if (selectedRequest?.id === reqId) setSelectedRequest({ ...selectedRequest, status: "Rejected", verificationStatus: "Rejected by BPLO Licensing Officer" });
    setActionFeedback({ type: "danger", message: `Application ${reqId} rejected.` });
    setTimeout(() => setActionFeedback(null), 4000);
  }

  function handleRequestCorrection(reqId) {
    const note = correctionNote || "Barangay Business Clearance is missing or expired. Please upload an updated clearance.";
    const updated = updateBPLORequestStatus(reqId, "Requires Correction", {
  ...
  function handleMarkManual(reqId) {
    const updated = updateBPLORequestStatus(reqId, "Under Manual Investigation", {
      verificationStatus: "Scheduled for BPLO Joint Inspection Team Site Audit",
    });
    setRequests(updated);
    if (selectedRequest?.id === reqId) {
      setSelectedRequest({
    ...
    setActionFeedback({ type: "warning", message: `Application ${reqId} assigned for site inspection.` });
    setTimeout(() => setActionFeedback(null), 4000);
  }

  return (
    <OfficeLayout office={bploOffice} header="Business Permits &amp; Licenses Console">
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between animate-fade-up">
          <div>
            <p className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.22em] text-gray-500">
              ...
              City Government of Malaybalay · BPLO Licensing Desk
            </p>
            <h1 className="mt-2 text-2xl font-extrabold tracking-tight text-gray-900 sm:text-3xl">BPLO Applications Console</h1>
          </div>
        ...
        <section className="grid animate-fade-up grid-cols-2 divide-y divide-gray-100 rounded-3xl border border-gray-200/70 bg-white shadow-sm sm:grid-cols-4 sm:divide-x sm:divide-y-0" style={{ animationDelay: "40ms" }}>
          {[
            { label: "Total Filings", value: stats.total, note: "Licensing queue", cls: "text-gray-900" },
            { label: "Approved & Issued", value: stats.approved, note: "Active business permits", cls: "text-emerald-600" },
            { label: "Pending Review", value: stats.pending, note: "Awaiting staff audit", cls: "text-blue-600" },
            { label: "Requires Correction", value: stats.requiresCorrection, note: "Missing clearances", cls: "text-amber-600" },
          ].map((s) => (
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between animate-fade-up" style={{ animationDelay: "60ms" }}>
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              placeholder="Search by ID, business name, owner…"
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0">
            ...
            >
              ...
              <option value="Retirement">Business Retirement</option>
            </select>
            {["All", "Ready for BPLO Verification", "Requires Correction", "Approved", "Rejected"].map((status) => (
              <button
                key={status}
            ...
            <table className="w-full min-w-240 text-left text-xs">
              <thead>
                <tr className="border-b border-gray-200 text-[11px] uppercase tracking-wider text-gray-500">
                  ...
                  <th className="px-3.5 py-3 font-semibold">Payment</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredRequests.length === 0 ? (
                  ...
                  filteredRequests.map((req) => {
                    ...
                    const isFree = req.payment?.status === "FREE";
                    return (
                      <tr key={req.id} className="transition hover:bg-gray-50/70">
                        ...
                        <td className="px-3.5 py-4 font-bold text-gray-900">{req.business?.businessName}</td>
                        <td className="px-3.5 py-4 text-gray-700">{req.owner?.fullName}</td>
                        ...
                        <td className="px-3.5 py-4">
                          {isPaid ? (
                            <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-700"><Check size={13} /> {req.payment.amount}</span>
                          ) : isFree ? (
                            <span className="inline-flex items-center gap-1 text-[11px] font-bold text-gray-600">FREE</span>
                          ) : (
                            <span className="inline-flex items-center gap-1 text-[11px] font-bold text-amber-700">Pending</span>
                          )}
                        </td>
            ...
            <div className="w-screen max-w-xl bg-white shadow-2xl">
              <div className="flex h-full flex-col overflow-y-auto">
                ...
                <div className="sticky top-0 z-20 flex items-center justify-between border-b border-zinc-200 bg-white px-6 py-4">
                  <div>
                    <span className="font-mono text-[11px] font-bold text-red-600">BPLO Licensing Officer Review</span>
                ...
                <div className="space-y-6 px-6 py-6 text-xs">
                  ...
                  <div className="flex items-center justify-between rounded-2xl bg-zinc-50 p-4 border border-zinc-200">
                    <div>
                      <span className="text-[10px] font-extrabold uppercase tracking-wider text-gray-400">BPLO Service</span>
                      <p className="font-extrabold text-gray-900 text-sm">{selectedRequest.certificateType}</p>
                    </div>
                    <span className="rounded-full bg-red-100 px-3 py-1 font-mono text-xs font-bold text-red-800">
                      {selectedRequest.payment?.amount || "FREE"}
                    </span>
                  </div>
                  ...
                  <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-xs">
                    <span className="text-[10px] font-extrabold uppercase tracking-wider text-red-600">Establishment Information</span>
                    <div className="mt-3 grid grid-cols-2 gap-3 text-gray-700">
                      <div><span className="text-gray-400 block text-[11px]">Business Name</span><span className="font-bold text-gray-900">{selectedRequest.business?.businessName}</span></div>
                      <div><span className="text-gray-400 block text-[11px]">Existing Permit No.</span><span className="font-medium text-gray-900">{selectedRequest.business?.existingPermitNumber || selectedRequest.business?.permitNumber || "—"}</span></div>
                      <div><span className="text-gray-400 block text-[11px]">Nature of Business</span><span className="font-medium text-gray-900">{selectedRequest.business?.nature || "—"}</span></div>
                      <div><span className="text-gray-400 block text-[11px]">Business Type</span><span className="font-medium text-gray-900">{selectedRequest.business?.businessType || "—"}</span></div>
                      {selectedRequest.business?.numberOfEmployees && <div><span className="text-gray-400 block text-[11px]">Employees</span><span className="font-medium text-gray-900">{selectedRequest.business.numberOfEmployees}</span></div>}
                      {selectedRequest.business?.registrationNumber && <div><span className="text-gray-400 block text-[11px]">Registration No.</span><span className="font-medium text-gray-900">{selectedRequest.business.registrationNumber}</span></div>}
                      {selectedRequest.business?.dateStopped && <div><span className="text-gray-400 block text-[11px]">Closure Date</span><span className="font-medium text-gray-900">{selectedRequest.business.dateStopped}</span></div>}
                      {selectedRequest.business?.dateRetirement && <div><span className="text-gray-400 block text-[11px]">Retirement Date</span><span className="font-medium text-gray-900">{selectedRequest.business.dateRetirement}</span></div>}
                      {selectedRequest.business?.reason && <div className="col-span-2"><span className="text-gray-400 block text-[11px]">Reason</span><span className="font-medium text-gray-900">{selectedRequest.business.reason}</span></div>}
                      <div className="col-span-2"><span className="text-gray-400 block text-[11px]">Business Address</span><span className="font-medium text-gray-900">{selectedRequest.business?.businessAddress}</span></div>
                    </div>
                  </div>

                  {/* Owner Details */}
                  <div className="rounded-2xl border border-zinc-200 bg-zinc-50/60 p-4">
                    <span className="text-[10px] font-extrabold uppercase tracking-wider text-gray-400">Business Owner / Signatory</span>
                    <div className="mt-3 grid grid-cols-2 gap-3 text-gray-700">
                      <div><span className="text-gray-400 block text-[11px]">Full Name</span><span className="font-bold text-gray-900">{selectedRequest.owner?.fullName}</span></div>
                      <div><span className="text-gray-400 block text-[11px]">Owner Type</span><span className="font-medium text-gray-900">{selectedRequest.owner?.ownerType || "Individual"}</span></div>
                      <div><span className="text-gray-400 block text-[11px]">Contact</span><span className="font-medium text-gray-900">{selectedRequest.owner?.contactNumber}</span></div>
                      <div><span className="text-gray-400 block text-[11px]">Email</span><span className="font-medium text-gray-900">{selectedRequest.owner?.email}</span></div>
                      <div className="col-span-2"><span className="text-gray-400 block text-[11px]">Owner Address</span><span className="font-medium text-gray-900">{selectedRequest.owner?.address}</span></div>
                    </div>
                  </div>

                  {/* Uploaded Documents */}
                  <div className="rounded-2xl border border-zinc-200 bg-zinc-50/60 p-4">
                    <span className="text-[10px] font-extrabold uppercase tracking-wider text-gray-400">Attached Clearances &amp; Documents</span>
                  ...
                  {/* Business Record Verification result */}
                  {selectedRequest.businessRecordCheck && (
                    <div className="rounded-2xl border border-blue-200 bg-blue-50/50 p-4">
                      <span className="text-[10px] font-extrabold uppercase tracking-wider text-blue-700">BPLO Registry Check</span>
                      <p className="mt-1 font-bold text-gray-900 text-xs">{selectedRequest.businessRecordCheck.message}</p>
                      {selectedRequest.businessRecordCheck.permitNumber && (
                        <p className="mt-1 font-mono text-[11px] text-blue-700">Permit No.: {selectedRequest.businessRecordCheck.permitNumber}</p>
                      )}
                    </div>
                  )}
                  ...
                  {selectedRequest.payment && (
                    <div className="rounded-2xl border border-zinc-200 bg-white p-4">
                      <span className="text-[10px] font-extrabold uppercase tracking-wider text-gray-500">Payment Status</span>
                      <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                        <div><span className="text-gray-400 block text-[11px]">Status</span><span className="font-bold text-emerald-700">{selectedRequest.payment.status}</span></div>
                        <div><span className="text-gray-400 block text-[11px]">Amount</span><span className="font-bold text-gray-900">{selectedRequest.payment.amount}</span></div>
                        {selectedRequest.payment.referenceNumber && (
                          <div className="col-span-2"><span className="text-gray-400 block text-[11px]">Reference No.</span><span className="font-mono text-gray-900">{selectedRequest.payment.referenceNumber}</span></div>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="rounded-2xl border border-zinc-200 bg-zinc-50/60 p-4">
                    ...
                    {selectedRequest.correctionNote && (
                      <div className="mt-3 rounded-xl border border-amber-300 bg-amber-50 p-3 text-amber-900">
                        <span className="font-bold block text-xs">Correction Note to Taxpayer:</span>
                ...
                <div className="sticky bottom-0 border-t border-zinc-200 bg-white px-6 py-4 space-y-2.5">
                  ...
                  <button onClick={() => handleMarkManual(selectedRequest.id)} className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-zinc-200 py-2 text-[11px] font-bold text-gray-600 hover:bg-zinc-50">
                    <ShieldCheck size={13} /> Mark for Joint Inspection Team Audit
                  </button>
                </div>
              </div>
            </div>
      ...
      {showCorrectionModal && selectedRequest && (
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl text-xs">
            <h3 className="text-sm font-extrabold text-gray-900">Request Clearance Correction</h3>
            <p className="mt-1 text-gray-500">Send an official notice to <span className="font-bold text-gray-800">{selectedRequest.owner?.fullName}</span> for <span className="font-bold text-gray-800">{selectedRequest.business?.businessName}</span>.</p>
            <textarea
              rows={4}
              value={correctionNote}
              onChange={(e) => setCorrectionNote(e.target.value)}
              placeholder="e.g. Barangay Business Clearance is missing or expired. Please upload an updated clearance from your respective Barangay Hall."
...
function StatusPill({ status }) {
  const styles = {
    ...
    "Ready for BPLO Verification": "bg-blue-50 text-blue-700 border-blue-200 animate-pulse",
