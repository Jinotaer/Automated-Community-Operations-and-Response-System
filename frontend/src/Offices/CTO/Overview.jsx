// src/Offices/CTO/Overview.jsx
import { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import {
  FileText, ArrowUpRight, Sparkles, Clock3, CheckCircle2, AlertCircle, RefreshCw, Wallet,
} from "lucide-react";
import OfficeLayout from "../OfficeLayout";
import { ctoOffice } from "../officeData";
import { getCTORequests } from "../../services/ctoData";
...
export default function CTOOverview() {
  ...
  useEffect(() => {
    document.title = "City Treasurer's Office Overview — ACORS";
    loadData();
  }, []);
  ...
  const statCards = [
    { title: "Total Requests", value: stats.total, note: "All CTO submissions" },
    { title: "Approved & Ready", value: stats.approved, note: "Certificates issued" },
    { title: "Requires Review", value: stats.reviewNeeded, note: "Flagged / outstanding" },
  ...
  return (
    <OfficeLayout office={ctoOffice} header="City Treasurer's Office Overview">
        ...
        <section className="grid gap-6 xl:grid-cols-12">
          <div className="animate-fade-up rounded-3xl border border-gray-200/70 bg-white p-5 shadow-sm xl:col-span-7" style={{ animationDelay: "80ms" }}>
            <div className="flex items-center justify-between">
              <h2 className="text-base font-extrabold text-gray-900 sm:text-lg">Certificate Services Volume</h2>
              <span className="font-mono text-xs text-gray-400">Past 30 Days</span>
            </div>
            <div className="mt-5 space-y-4">
              {ctoOffice.categoryBreakdown.map((item) => (
            ...
            <div className="mt-6 rounded-2xl bg-zinc-50 p-4 text-xs text-gray-600">
              ...
              <p className="mt-1 text-[11px] text-gray-500">
                Tax clearance requests with no outstanding balance are matched and processed automatically without manual archive lookup.
              </p>
            </div>
          </div>
          ...
          <div className="animate-fade-up rounded-3xl border border-gray-200/70 bg-white p-5 shadow-sm xl:col-span-5" style={{ animationDelay: "120ms" }}>
            <h2 className="text-base font-extrabold text-gray-900 sm:text-lg">Queue Breakdown</h2>
            <div className="mt-5 space-y-4 text-xs">
              <div className="flex items-center justify-between rounded-2xl border border-emerald-100 bg-emerald-50/50 p-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div><span className="block font-bold text-gray-900">Auto-Approved / Ready</span><span className="text-[11px] text-gray-500">Ready for citizen download</span></div>
                </div>
              ...
              <div className="flex items-center justify-between rounded-2xl border border-amber-100 bg-amber-50/50 p-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div><span className="block font-bold text-gray-900">Requires Staff Review</span><span className="text-[11px] text-gray-500">Outstanding balance or mismatch</span></div>
                </div>
              ...
              <div className="flex items-center justify-between rounded-2xl border border-zinc-200 bg-zinc-50 p-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div><span className="block font-bold text-gray-900">Processing & Payment</span><span className="text-[11px] text-gray-500">Active submission pipeline</span></div>
                </div>
                ...
                {requests.slice(0, 6).map((req) => {
                  ...
                  return (
                    <tr key={req.id} className="transition hover:bg-gray-50/70">
                      ...
                      <td className="px-3.5 py-3.5">
                        <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 font-mono text-[10px] font-bold ${req.aiValidation?.status === "PASSED" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>
