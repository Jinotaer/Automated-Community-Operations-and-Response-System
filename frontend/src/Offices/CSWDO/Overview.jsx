// src/Offices/CSWDO/Overview.jsx
import { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import {
  FileText, ArrowUpRight, Sparkles, Clock3, CheckCircle2, AlertCircle, RefreshCw, HeartHandshake,
} from "lucide-react";
import OfficeLayout from "../OfficeLayout";
import { cswdoOffice } from "../officeData";
import { getSoloParentRequests } from "../../services/soloParentData";

export default function CSWDOOverview() {
  const [requests, setRequests] = useState([]);

  const loadData = () => setRequests(getSoloParentRequests());

  useEffect(() => {
    document.title = "CSWDO / Solo Parent Office Overview — ACORS";
    loadData();
  }, []);
  ...
  const statCards = [
    { title: "Total Applications", value: stats.total, note: "All Solo Parent filings" },
    { title: "Pending Verification", value: stats.pending, note: "Awaiting social worker" },
    { title: "AI Passed", value: stats.aiPassed, note: "Complete documentation" },
    { title: "Requires Correction", value: stats.requiresCorrection, note: "Incomplete proof" },
    { title: "Approved & Issued", value: stats.approved, note: "Registered solo parents" },
  ];
  ...
  return (
    <OfficeLayout office={cswdoOffice} header="Solo Parent Office / CSWDO">
      <div className="space-y-5 sm:space-y-6">
        ...
        <header className="flex animate-fade-up flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.22em] text-gray-500">
              ...
              City Government of Malaybalay · CSWDO Solo Parent Welfare Portal
            </p>
          ...
          <div className="flex items-center gap-2">
            ...
            <Link to="/department/soloparent/requests" className="flex items-center gap-2 rounded-xl bg-red-600 px-4 py-2.5 text-xs font-bold text-white shadow-sm transition hover:bg-red-700">
        ...
        <section className="grid gap-6 xl:grid-cols-12">
          <div className="animate-fade-up rounded-3xl border border-gray-200/70 bg-white p-5 shadow-sm xl:col-span-7" style={{ animationDelay: "80ms" }}>
            <div className="flex items-center justify-between">
              <h2 className="text-base font-extrabold text-gray-900 sm:text-lg">Services Volume Breakdown</h2>
              <span className="font-mono text-xs text-gray-400">RA 8972 &amp; RA 11861</span>
            </div>
            <div className="mt-5 space-y-4">
              {cswdoOffice.categoryBreakdown.map((item) => (
            ...
            <div className="mt-6 rounded-2xl bg-zinc-50 p-4 text-xs text-gray-600">
              ...
              <p className="mt-1 text-[11px] text-gray-500">
                AI scans for complete birth certificates, barangay residency proof, and status evidence consistency. Official welfare qualifications are verified by licensed CSWDO social workers.
              </p>
            </div>
          </div>
            ...
            <div className="mt-5 space-y-4 text-xs">
              <div className="flex items-center justify-between rounded-2xl border border-emerald-100 bg-emerald-50/50 p-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div><span className="block font-bold text-gray-900">Approved & Issued</span><span className="text-[11px] text-gray-500">IDs and certificates issued</span></div>
                </div>
              ...
              <div className="flex items-center justify-between rounded-2xl border border-blue-100 bg-blue-50/50 p-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div><span className="block font-bold text-gray-900">Pending Verification</span><span className="text-[11px] text-gray-500">Social worker assessment</span></div>
                </div>
              ...
              <div className="flex items-center justify-between rounded-2xl border border-amber-100 bg-amber-50/50 p-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div><span className="block font-bold text-gray-900">Requires Correction</span><span className="text-[11px] text-gray-500">Missing solo parent proof</span></div>
                </div>
                <span className="font-mono text-lg font-bold text-amber-700">{stats.requiresCorrection}</span>
              </div>
            </div>
            <Link to="/department/soloparent/requests" className="mt-6 flex w-full items-center justify-center gap-1.5 rounded-xl border border-zinc-200 py-3 text-xs font-bold text-gray-700 transition hover:bg-zinc-50">
        ...
        <section className="animate-fade-up rounded-3xl border border-gray-200/70 bg-white p-5 shadow-sm" style={{ animationDelay: "160ms" }}>
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-extrabold text-gray-900 sm:text-lg">Recent Solo Parent Filings</h2>
              <p className="text-xs text-gray-500">Applications from citizens across Malaybalay City</p>
            </div>
            <Link to="/department/soloparent/requests" className="text-xs font-bold text-red-600 hover:text-red-700">View Full Table</Link>
          </div>
            ...
            <table className="w-full min-w-180 text-left text-xs">
              <thead>
                <tr className="border-b border-gray-200 text-[11px] uppercase tracking-wider text-gray-500">
                  ...
                  <th className="px-3.5 py-3 font-semibold">Reason</th>
                  <th className="px-3.5 py-3 font-semibold">AI Status</th>
                ...
                {requests.slice(0, 6).map((req) => {
                  ...
                  return (
                    <tr key={req.id} className="transition hover:bg-gray-50/70">
                      ...
                      <td className="px-3.5 py-3.5 text-gray-700">{reason}</td>
                      <td className="px-3.5 py-3.5">
