// src/Offices/OSCA/Overview.jsx
import { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import {
  FileText, ArrowUpRight, Sparkles, Clock3, CheckCircle2, AlertCircle, RefreshCw, Users, Heart,
} from "lucide-react";
import OfficeLayout from "../OfficeLayout";
import { oscaOffice } from "../officeData";
import { getOSCARequests } from "../../services/oscaData";

export default function OSCAOverview() {
  const [requests, setRequests] = useState([]);

  const loadData = () => setRequests(getOSCARequests());

  useEffect(() => {
    document.title = "Office for Senior Citizens Affairs Overview — ACORS";
    loadData();
  }, []);

  const stats = {
    total: requests.length,
    seniorId: requests.filter((r) => r.certificateType === "Senior Citizen ID").length,
    regCert: requests.filter((r) => r.certificateType?.includes("Registration")).length,
    cert: requests.filter((r) => r.certificateType?.includes("Certification")).length,
  ...
  const statCards = [
    { title: "Total Applications", value: stats.total, note: "All OSCA filings" },
    { title: "Senior Citizen IDs", value: stats.seniorId, note: "RA 9994 IDs" },
    { title: "Pending Review", value: stats.pending, note: "Awaiting staff audit" },
    { title: "AI Passed & OCR", value: stats.aiPassed, note: "Verified 60+ age" },
    { title: "Approved & Issued", value: stats.approved, note: "Active senior IDs & certs" },
  ];
  ...
  return (
    <OfficeLayout office={oscaOffice} header="Office for Senior Citizens Affairs">
      <div className="space-y-5 sm:space-y-6">
        ...
        <header className="flex animate-fade-up flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.22em] text-gray-500">
              ...
              City Government of Malaybalay · OSCA Senior Services Desk
            </p>
          ...
          <div className="flex items-center gap-2">
            ...
            <Link to="/department/osca/requests" className="flex items-center gap-2 rounded-xl bg-red-600 px-4 py-2.5 text-xs font-bold text-white shadow-sm transition hover:bg-red-700">
        ...
        <section className="grid gap-6 xl:grid-cols-12">
          <div className="animate-fade-up rounded-3xl border border-gray-200/70 bg-white p-5 shadow-sm xl:col-span-7" style={{ animationDelay: "80ms" }}>
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-xs text-gray-400">Senior Welfare Filings</span>
            </div>
            <div className="mt-5 space-y-4">
              {oscaOffice.categoryBreakdown.map((item) => (
            ...
            <div className="mt-6 rounded-2xl bg-zinc-50 p-4 text-xs text-gray-600">
              <div className="flex items-center gap-2 font-bold text-gray-900">
                ...
                <span>AI OCR &amp; Age Pre-Screening Active</span>
              </div>
              <p className="mt-1 text-[11px] text-gray-500">
                Simulated OCR extracts names, birth dates, and proofs of age (60+ years). Final senior citizen eligibility is verified and approved by authorized OSCA officers.
              </p>
            </div>
          </div>
            ...
            <div className="mt-5 space-y-4 text-xs">
              <div className="flex items-center justify-between rounded-2xl border border-emerald-100 bg-emerald-50/50 p-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div><span className="block font-bold text-gray-900">Approved & Issued</span><span className="text-[11px] text-gray-500">IDs and certificates sealed</span></div>
                </div>
              ...
              <div className="flex items-center justify-between rounded-2xl border border-blue-100 bg-blue-50/50 p-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div><span className="block font-bold text-gray-900">Pending Verification</span><span className="text-[11px] text-gray-500">Awaiting OSCA staff review</span></div>
                </div>
              ...
              <div className="flex items-center justify-between rounded-2xl border border-amber-100 bg-amber-50/50 p-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div><span className="block font-bold text-gray-900">Requires Correction</span><span className="text-[11px] text-gray-500">Missing residence proof</span></div>
                </div>
                <span className="font-mono text-lg font-bold text-amber-700">{stats.requiresCorrection}</span>
              </div>
            </div>
            <Link to="/department/osca/requests" className="mt-6 flex w-full items-center justify-center gap-1.5 rounded-xl border border-zinc-200 py-3 text-xs font-bold text-gray-700 transition hover:bg-zinc-50">
        ...
        <section className="animate-fade-up rounded-3xl border border-gray-200/70 bg-white p-5 shadow-sm" style={{ animationDelay: "160ms" }}>
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-extrabold text-gray-900 sm:text-lg">Recent Senior Citizen Applications</h2>
              <p className="text-xs text-gray-500">Senior welfare and ID filings in Malaybalay City</p>
            </div>
            <Link to="/department/osca/requests" className="text-xs font-bold text-red-600 hover:text-red-700">View Full Table</Link>
          </div>
                ...
                {requests.slice(0, 6).map((req) => {
                  ...
                  return (
                    <tr key={req.id} className="transition hover:bg-gray-50/70">
                      ...
                      <td className="px-3.5 py-3.5 font-mono text-gray-600">{req.applicant?.dob}</td>
                      <td className="px-3.5 py-3.5">
