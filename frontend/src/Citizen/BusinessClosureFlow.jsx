// src/Citizen/BusinessClosureFlow.jsx
...
export default function BusinessClosureFlow({ office, cert }) {
  ...
  const [permitFile, setPermitFile] = useState({ name: "business_permit_2025.pdf", type: "application/pdf", size: "1.8 MB", uploaded: true });
  const [regFile, setRegFile] = useState({ name: "dti_certificate.pdf", type: "application/pdf", size: "1.5 MB", uploaded: true });
  const [brgyClearanceFile, setBrgyClearanceFile] = useState({ name: "brgy_closure_clearance.pdf", type: "application/pdf", size: "0.9 MB", uploaded: true });
  ...
  function handleSubmit() {
    ...
    const certNo = `BC-2026-${Math.floor(100000 + Math.random() * 900000)}`;
    ...
    saveBPLORequest({
      ...
      aiValidation: {
        ...
        confidence: "94%",
        checks: aiChecksDef,
        recommendation: "Closure documentation verified. Awaiting official BPLO recording.",
      },
      businessRecordCheck: matchedRecord
        ? { status: "MATCHED", permitNumber: matchedRecord.permitNumber, message: "Business Found & Permit Found (Active Status)." }
      ...
      documents: [
        ...
        { name: "Business Permit", fileName: permitFile.name, verified: permitFile.uploaded },
        { name: "Barangay Clearance for Closure", fileName: brgyClearanceFile.name, verified: brgyClearanceFile.uploaded },
      ],
    });
      ...
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <h2 className="text-base font-extrabold text-gray-900 sm:text-lg">
            Certificate of Business Closure Guidelines
          </h2>
          <p className="mt-1.5 text-xs leading-relaxed text-gray-500 sm:text-sm">
            Submit an official request to formally record the closure of a registered business in Malaybalay City.
          </p>
          ...
          <div className="mt-5 grid grid-cols-2 gap-3 text-xs">
            <div className="rounded-2xl border border-zinc-100 bg-zinc-50/80 p-3.5">
              ...
              <p className="text-[11px] text-gray-400">Formal BPLO recording</p>
            </div>
            <div className="rounded-2xl border border-zinc-100 bg-zinc-50/80 p-3.5">
              <span className="font-bold text-gray-800">Record Verification</span>
              <p className="mt-1 font-mono text-base font-extrabold text-gray-900">Automated</p>
              <p className="text-[11px] text-gray-400">BPLO database check</p>
            </div>
          </div>
          ...
          <div className="mt-5 rounded-2xl bg-zinc-50 p-4">
            ...
            {[
              ...
              "Existing/Latest Business Permit",
              "Business Registration Document (DTI / SEC)",
          ...
          <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-3.5 text-xs text-amber-800 flex items-start gap-2">
            ...
            <span>
              IMPORTANT: Closure records require final sign-off and tax clearance confirmation by BPLO and City Treasurer's Office.
            </span>
          </div>
      ...
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <div>
              <h2 className="text-sm font-extrabold text-gray-900 sm:text-base">Business &amp; Closure Information</h2>
              <p className="text-xs text-gray-400">Specify details of the establishment closing</p>
            </div>
          </div>
          <div className="space-y-4">
            <div>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                <label className="mb-1.5 block text-xs font-bold text-gray-700">Date Business Stopped Operations</label>
                <input
                  type="date"
                  value={business.dateStopped}
                  onChange={(e) => setBusiness((p) => ({ ...p, dateStopped: e.target.value }))}
                  className="w-full rounded-xl border border-zinc-200 bg-zinc-50/50 px-3.5 py-2.5 text-xs text-gray-900 focus:border-red-600 focus:bg-white focus:outline-none focus:ring-1 focus:ring-red-600"
                />
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-bold text-gray-700">Reason for Closure</label>
                <select
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Attachments for closure recording</p>
            </div>
          </div>
          <div className="space-y-3">
            ...
            <UploadBox label="Business Permit (Latest / 2025)" file={permitFile} onUpload={() => setPermitFile({ name: "business_permit_2025.pdf", type: "application/pdf", size: "1.8 MB", uploaded: true })} onRemove={() => setPermitFile({ name: "", uploaded: false })} required />
            <UploadBox label="Business Registration Document (DTI / SEC)" file={regFile} onUpload={() => setRegFile({ name: "dti_certificate.pdf", type: "application/pdf", size: "1.5 MB", uploaded: true })} onRemove={() => setRegFile({ name: "", uploaded: false })} />
            <UploadBox label="Barangay Clearance for Business Closure" file={brgyClearanceFile} onUpload={() => setBrgyClearanceFile({ name: "brgy_closure_clearance.pdf", type: "application/pdf", size: "0.9 MB", uploaded: true })} onRemove={() => setBrgyClearanceFile({ name: "", uploaded: false })} required />
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Closure consistency and permit check</p>
            </div>
          </div>
          ...
          {aiChecks.length === 0 && !aiScanning ? (
            <div className="flex flex-col items-center gap-3 py-8 text-center">
              ...
              <p className="text-sm font-bold text-gray-800">Ready to validate closure request</p>
              <p className="max-w-xs text-xs text-gray-500">AI will scan for required barangay closure clearance and cross-verify existing permit numbers.</p>
              <button
                type="button"
      ...
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <div>
              <h2 className="text-sm font-extrabold text-gray-900 sm:text-base">Mock Business Record Check</h2>
          ...
          <div className="rounded-2xl border border-zinc-100 bg-zinc-50 p-4 text-xs mb-5 space-y-2">
            ...
            <SummaryRow label="Reason for Closure" value={business.reason} />
          </div>
          ...
          {matchedRecord === null && !isVerifying && (
            ...
            >
              <ShieldCheck size={16} /> Check BPLO Business Records
            </button>
          )}
          ...
          {isVerifying && (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="text-xs sm:text-sm font-bold text-gray-700">Checking business status in database…</p>
            </div>
          )}
          ...
          {!isVerifying && matchedRecord && typeof matchedRecord === "object" && (
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 space-y-2 text-xs">
              <div className="flex items-center gap-2 font-extrabold text-emerald-800 mb-3">
                ...
                <span>Business Found &amp; Permit Status Verified</span>
              </div>
              ...
              <SummaryRow label="Current Status" value={matchedRecord.status} />
            </div>
          )}
          ...
          {!isVerifying && matchedRecord === false && (
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs">
              ...
              <p className="text-amber-700">Business record could not be matched automatically. Application will be routed to BPLO licensing officer for manual audit.</p>
            </div>
          )}
        </div>
    ...
    return (
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6 space-y-3">
          ...
          <SummaryRow label="Service" value="Certificate of Business Closure" />
          <SummaryRow label="Owner Name" value={owner.fullName} />
          <SummaryRow label="Business Name" value={business.businessName} />
          <SummaryRow label="Permit Number" value={business.permitNumber} />
          <SummaryRow label="Business Address" value={business.businessAddress} />
          <SummaryRow label="Closure Date" value={business.dateStopped} />
          <SummaryRow label="Reason for Closure" value={business.reason} />
          ...
          <div className={`rounded-xl border px-3.5 py-2.5 text-xs font-bold ${matchedRecord && typeof matchedRecord === "object" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-50 text-amber-800"}`}>
            Business Record: {matchedRecord && typeof matchedRecord === "object" ? "VERIFIED (ACTIVE PERMIT)" : "REQUIRES BPLO REVIEW"}
          </div>
        </div>
        <NavButtons onBack={back} onNext={handleSubmit} nextLabel="Submit Business Closure Request" />
      </div>
    );
  }
  ...
  function StepTracking() {
    const trackSteps = [
      ...
      { label: "Closure Recorded", done: false },
    ...
    return (
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <h2 className="text-sm font-extrabold text-gray-900 mb-1">Business Closure Tracking</h2>
          ...
          <div className="mb-5 rounded-2xl border border-blue-200 bg-blue-50/60 p-4 text-xs">
            ...
            <p className="mt-1 text-gray-600">Your closure request has been submitted and is queued for recording and clearance certification by BPLO.</p>
          </div>
    ...
    return (
      <div className="space-y-5 animate-fade-up">
        ...
        <div className="rounded-3xl border border-emerald-200 bg-emerald-50/40 p-5 text-center">
          ...
          <p className="text-base sm:text-lg font-extrabold text-gray-900">Certificate of Business Closure Ready</p>
        ...
        <div className="relative overflow-hidden rounded-3xl border-2 border-zinc-200 bg-white p-5 sm:p-6 shadow-sm">
          ...
          <div className="relative z-10">
            <div className="text-center border-b border-zinc-200 pb-4 mb-4">
              ...
              <p className="mt-2 text-xs font-extrabold uppercase tracking-[0.2em] text-red-700">Certificate of Business Closure</p>
              <p className="font-mono text-xs font-bold text-gray-700 mt-1">{issuedCertNo}</p>
            </div>

            <div className="space-y-2 text-xs">
              <SummaryRow label="Business Name" value={business.businessName} />
              <SummaryRow label="Registered Owner" value={owner.fullName} />
              <SummaryRow label="Business Address" value={business.businessAddress} />
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-sm rounded-3xl bg-white p-6 shadow-2xl">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-sm font-extrabold text-gray-900">QR Closure Verification</h3>
          ...
          <div className="space-y-2 text-xs border-t border-zinc-100 pt-4">
            ...
            <SummaryRow label="Document" value="Certificate of Business Closure" />
          ...
          <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 py-2 text-center">
            <span className="text-xs font-extrabold text-emerald-700">VALID — BPLO Registry Recorded</span>
          </div>
  ...
  const stepComponents = [
    <StepStart />, <StepOwnerInfo />, <StepClosureInfo />, <StepUploadDocs />,
      ...
      <div className="mt-4 flex items-center gap-3.5 rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm sm:p-5 animate-fade-up">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-xs text-gray-500">
            Official closure certification · Formal BPLO recording
          </p>
        </div>
      </div>
