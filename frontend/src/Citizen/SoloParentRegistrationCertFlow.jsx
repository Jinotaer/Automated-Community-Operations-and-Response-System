// src/Citizen/SoloParentRegistrationCertFlow.jsx
...
import { saveSoloParentRequest, findMockSoloParentRegistration } from "../services/soloParentData";
...
export default function SoloParentRegistrationCertFlow({ office, cert }) {
  ...
  const [applicant, setApplicant] = useState({
    ...
    dob: "1986-11-04",
    address: "Purok 5, Sumpong, Malaybalay City, Bukidnon",
    barangay: "Sumpong",
  ...
  const [idFile, setIdFile] = useState({ name: "umid_delacruz.jpg", type: "image/jpeg", size: "1.3 MB", uploaded: true });
  const [spIdFile, setSpIdFile] = useState({ name: "spid_delacruz_cert.pdf", type: "application/pdf", size: "1.1 MB", uploaded: true });
  const [supportFile, setSupportFile] = useState({ name: "affidavit_abandonment.pdf", type: "application/pdf", size: "0.8 MB", uploaded: true });
  ...
  const aiChecksDef = [
    ...
    { label: "Registration information consistency", passed: testMode === "normal" },
  ...
  function runRegistrationCheck() {
    ...
    setTimeout(() => {
      const found = testMode === "normal"
        ? findMockSoloParentRegistration({ name: applicant.fullName, registrationNumber: registrationInfo.registrationNumber })
        : null;
  ...
  function handleSubmit() {
    ...
    const certNo = `SP-CRSP-2026-${Math.floor(100000 + Math.random() * 900000)}`;
    ...
    saveSoloParentRequest({
      ...
      registryVerification: matchedRecord
        ? { status: "MATCHED", registryId: matchedRecord.registryId, message: "Registration Record Found & Verified. Active Status." }
        : { status: "UNMATCHED", message: "Registration record not found. Scheduled for manual archive assessment." },
      aiValidation: {
        status: aiStatus,
        confidence: "95%",
        checks: aiChecksDef,
        recommendation: "Registration documents verified. Ready for authorized CSWDO processing.",
      },
      ...
      documents: [
        ...
        { name: "Solo Parent ID / Proof", fileName: spIdFile.name, verified: spIdFile.uploaded },
        { name: "Supporting Affidavit", fileName: supportFile.name, verified: supportFile.uploaded },
      ],
    });
      ...
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <p className="mt-1.5 text-xs leading-relaxed text-gray-500 sm:text-sm">
            Request an official certificate confirming your registration in the CSWDO Masterlist of Solo Parents.
          </p>
          ...
          <div className="mt-5 grid grid-cols-2 gap-3 text-xs">
            <div className="rounded-2xl border border-zinc-100 bg-zinc-50/80 p-3.5">
              ...
              <p className="text-[11px] text-gray-400">Official CSWDO masterlist certification</p>
            </div>
            <div className="rounded-2xl border border-zinc-100 bg-zinc-50/80 p-3.5">
              ...
              <p className="text-[11px] text-gray-400">Masterlist registration lookup</p>
            </div>
          </div>
          ...
          <div className="mt-5 rounded-2xl bg-zinc-50 p-4">
            ...
            {[
              "Valid Government ID",
              "Solo Parent Registration / ID Number",
              "Declared number of dependent children",
              "Reason for solo parenthood and purpose of request",
            ].map((r) => (
          ...
          <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-3.5 text-xs text-amber-800 flex items-start gap-2">
            ...
            <span>
              IMPORTANT: This checks your registration against sample CSWDO records. Final certification is signed by authorized CSWDO personnel.
            </span>
          </div>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2.5">
            ...
            {[
              ...
              { key: "unmatched", label: "Not Found (Review)" },
            ].map((m) => (
              <button
                key={m.key}
                type="button"
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Masterlist registrant identity</p>
            </div>
          </div>
          <div className="space-y-4">
            {[
              ...
              { key: "barangay", label: "Barangay", type: "select", options: ["Sumpong", "Poblacion", "Casisang", "Kalasungay", "Bangcud", "Aglayan", "Other"] },
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Masterlist filing details</p>
            </div>
          </div>
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                <label className="mb-1.5 block text-xs font-bold text-gray-700">Registration Number</label>
                <input
                  type="text"
                  value={registrationInfo.registrationNumber}
                  onChange={(e) => setRegistrationInfo((p) => ({ ...p, registrationNumber: e.target.value }))}
                  placeholder="e.g. SP-REG-2026-001"
                  className="w-full rounded-xl border border-zinc-200 bg-zinc-50/50 px-3.5 py-2.5 text-xs text-gray-900 focus:border-red-600 focus:bg-white focus:outline-none focus:ring-1 focus:ring-red-600"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setRegistrationInfo((p) => ({ ...p, registrationDate: e.target.value }))}
                  className="w-full rounded-xl border border-zinc-200 bg-zinc-50/50 px-3.5 py-2.5 text-xs text-gray-900 focus:border-red-600 focus:bg-white focus:outline-none focus:ring-1 focus:ring-red-600"
                />
              </div>
            </div>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                ...
                <input
                  type="number"
                  min="1"
                  value={registrationInfo.numberOfChildren}
                  onChange={(e) => setRegistrationInfo((p) => ({ ...p, numberOfChildren: e.target.value }))}
                  className="w-full rounded-xl border border-zinc-200 bg-zinc-50/50 px-3.5 py-2.5 text-xs text-gray-900 focus:border-red-600 focus:bg-white focus:outline-none focus:ring-1 focus:ring-red-600"
                />
              </div>
              <div>
                ...
                <select
                  value={registrationInfo.reason}
                  onChange={(e) => setRegistrationInfo((p) => ({ ...p, reason: e.target.value }))}
                  className="w-full rounded-xl border border-zinc-200 bg-zinc-50/50 px-3.5 py-2.5 text-xs text-gray-900 focus:border-red-600 focus:bg-white focus:outline-none focus:ring-1 focus:ring-red-600"
                >
            ...
            <div>
              <label className="mb-1.5 block text-xs font-bold text-gray-700">Purpose of Request</label>
              <select
                value={registrationInfo.purpose}
                onChange={(e) => setRegistrationInfo((p) => ({ ...p, purpose: e.target.value }))}
                className="w-full rounded-xl border border-zinc-200 bg-zinc-50/50 px-3.5 py-2.5 text-xs text-gray-900 focus:border-red-600 focus:bg-white focus:outline-none focus:ring-1 focus:ring-red-600"
              >
      ...
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="space-y-3">
            <UploadBox label="Valid Government ID of Parent" file={idFile} onUpload={() => setIdFile({ name: "umid_delacruz.jpg", type: "image/jpeg", size: "1.3 MB", uploaded: true })} onRemove={() => setIdFile({ name: "", uploaded: false })} required />
            <UploadBox label="Solo Parent ID / Registration Card (if applicable)" file={spIdFile} onUpload={() => setSpIdFile({ name: "spid_delacruz_cert.pdf", type: "application/pdf", size: "1.1 MB", uploaded: true })} onRemove={() => setSpIdFile({ name: "", uploaded: false })} />
            <UploadBox label="Supporting Document / Barangay Proof" file={supportFile} onUpload={() => setSupportFile({ name: "affidavit_abandonment.pdf", type: "application/pdf", size: "0.8 MB", uploaded: true })} onRemove={() => setSupportFile({ name: "", uploaded: false })} />
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          {aiChecks.length === 0 && !aiScanning ? (
            <div className="flex flex-col items-center gap-3 py-8 text-center">
              ...
              <p className="max-w-xs text-xs text-gray-500">AI will verify ID authenticity, registration information, and scan for required attachments.</p>
              <button
                type="button"
              ...
              >
                <Sparkles size={14} /> Run Validation
              </button>
            </div>
          ) : (
            <>
  ...
  function StepRegistrationCheck() {
    return (
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <div>
              <h2 className="text-sm font-extrabold text-gray-900 sm:text-base">Mock Registry Check</h2>
              <p className="text-xs text-gray-400">Match registration against CSWDO Masterlist</p>
            </div>
          </div>

          <div className="rounded-2xl border border-zinc-100 bg-zinc-50 p-4 text-xs mb-5 space-y-2">
            <p className="font-extrabold text-gray-700 uppercase tracking-wider text-[10px] mb-2">Submitted Masterlist Entry</p>
            <SummaryRow label="Registrant Name" value={applicant.fullName} />
            <SummaryRow label="Registration No." value={registrationInfo.registrationNumber} />
            <SummaryRow label="Registration Date" value={registrationInfo.registrationDate} />
            <SummaryRow label="Number of Children" value={`${registrationInfo.numberOfChildren} Child/Children`} />
          </div>
          ...
          {matchedRecord === null && !isVerifying && (
            ...
            >
              <ShieldCheck size={16} /> Check CSWDO Masterlist Records
            </button>
          )}
          ...
          {isVerifying && (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="text-xs sm:text-sm font-bold text-gray-700">Querying masterlist records…</p>
            </div>
          )}
          ...
          {!isVerifying && matchedRecord && typeof matchedRecord === "object" && (
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 space-y-2 text-xs">
              <div className="flex items-center gap-2 font-extrabold text-emerald-800 mb-3">
                ...
                <span>Registration Found &amp; Information Matched</span>
              </div>
              <SummaryRow label="Registration Number" value={matchedRecord.registrationNumber || matchedRecord.registryId} />
              <SummaryRow label="Name" value={matchedRecord.name} />
              <SummaryRow label="Registration Status" value="ACTIVE" />
              <SummaryRow label="Registration Date" value={matchedRecord.registrationDate} />
              <SummaryRow label="Number of Children" value={`${matchedRecord.numberOfChildren} Child/Children`} />
            </div>
          )}
          ...
          {!isVerifying && matchedRecord === false && (
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs">
              ...
              <p className="text-amber-700">Record not found in the active masterlist. This request will be forwarded to CSWDO for archive case verification.</p>
            </div>
          )}
        </div>
    ...
    return (
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6 space-y-3">
          ...
          <SummaryRow label="Certificate" value="Certificate of Registration as Solo Parent" />
          <SummaryRow label="Applicant" value={applicant.fullName} />
          <SummaryRow label="Registration Number" value={registrationInfo.registrationNumber} />
          <SummaryRow label="Registration Date" value={registrationInfo.registrationDate} />
          <SummaryRow label="Number of Children" value={`${registrationInfo.numberOfChildren} Child/Children`} />
          <SummaryRow label="Purpose" value={registrationInfo.purpose} />
          <SummaryRow label="Status" value={matchedRecord && typeof matchedRecord === "object" ? "ACTIVE" : "PENDING CSWDO REVIEW"} />
          ...
          <div className={`rounded-xl border px-3.5 py-2.5 text-xs font-bold ${matchedRecord && typeof matchedRecord === "object" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-50 text-amber-800"}`}>
            Registry Verification: {matchedRecord && typeof matchedRecord === "object" ? "ACTIVE REGISTRATION MATCHED" : "REQUIRES CSWDO REVIEW"}
          </div>
        </div>
  ...
  function StepTracking() {
    ...
    const trackSteps = [
      ...
      { label: "Registration Verification", done: true },
      ...
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="mb-5 rounded-2xl border border-blue-200 bg-blue-50/60 p-4 text-xs">
            ...
            <p className="mt-1 text-gray-600">Your registration has been confirmed and is queued for official certificate preparation.</p>
          </div>
    ...
    return (
      <div className="space-y-5 animate-fade-up">
        ...
        <div className="rounded-3xl border border-emerald-200 bg-emerald-50/40 p-5 text-center">
          ...
          <p className="text-base sm:text-lg font-extrabold text-gray-900">Certificate of Registration as Solo Parent Ready</p>
          <p className="text-xs text-gray-500 mt-1">Official masterlist certification issued by CSWDO Malaybalay City.</p>
        ...
        <div className="relative overflow-hidden rounded-3xl border-2 border-zinc-200 bg-white p-5 sm:p-6 shadow-sm">
          ...
          <div className="relative z-10">
            <div className="text-center border-b border-zinc-200 pb-4 mb-4">
              ...
              <p className="mt-2 text-xs font-extrabold uppercase tracking-[0.2em] text-red-700">Certificate of Registration as Solo Parent</p>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-sm rounded-3xl bg-white p-6 shadow-2xl">
          ...
          <div className="space-y-2 text-xs border-t border-zinc-100 pt-4">
            ...
            <SummaryRow label="Document" value="Certificate of Registration as Solo Parent" />
          ...
          <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 py-2 text-center">
            <span className="text-xs font-extrabold text-emerald-700">VALID — CSWDO Masterlist Verified</span>
          </div>
  ...
  const stepComponents = [
    <StepStart />, <StepRequesterInfo />, <StepRegistrationInfo />, <StepUploadDocs />,
    <StepAIValidation />, <StepRegistrationCheck />, <StepSummary />, <StepTracking />, <StepCertReady />,
  ];
      ...
      <div className="mt-4 flex items-center gap-3.5 rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm sm:p-5 animate-fade-up">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-xs text-gray-500">
            Free masterlist certification · CSWDO database validation
          </p>
        </div>
      </div>
