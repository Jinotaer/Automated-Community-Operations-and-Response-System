// src/Citizen/PropertyAssessmentCertFlow.jsx
...
export default function PropertyAssessmentCertFlow({ office, cert }) {
  ...
  const [tdFile, setTdFile] = useState({ name: "tax_dec_santos.pdf", type: "application/pdf", size: "1.6 MB", uploaded: true });
  const [ownershipFile, setOwnershipFile] = useState({ name: "deed_of_sale_santos.pdf", type: "application/pdf", size: "1.9 MB", uploaded: true });
  ...
  const aiChecksDef = [
    ...
    { label: "Tax Declaration Number consistency", passed: testMode === "normal" },
    { label: "Property ID consistency", passed: testMode === "normal" },
    { label: "Possible duplicate request", passed: true },
  ];

  const aiStatus = testMode === "normal" ? "READY FOR ASSESSOR VERIFICATION" : "REQUIRES ASSESSOR REVIEW";
  ...
  function handleSubmit() {
    ...
    const certNo = `PAC-2026-${Math.floor(100000 + Math.random() * 900000)}`;
    ...
    saveAssessorRequest({
      ...
      property: {
        ...property,
        classification: matchedRecord?.classification || "Residential (Class B)",
        assessedValue: matchedRecord?.assessedValue || "₱750,000.00",
      },
      aiValidation: {
        status: aiStatus,
        confidence: "95%",
      ...
      propertyRecordCheck: matchedRecord
        ? { status: "MATCHED", propertyId: property.propertyId, taxDeclarationNumber: property.taxDeclarationNumber, message: "Property Record Found & Assessment Record Verified." }
        : { status: "UNMATCHED", message: "Record not automatically matched. Forwarded to assessor officer." },
      payment: {
        status: "Paid",
        amount: "₱150.00",
      ...
      documents: [
        ...
        { name: "Tax Declaration Copy", fileName: tdFile.name, verified: tdFile.uploaded },
      ...
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <h2 className="text-base font-extrabold text-gray-900 sm:text-lg">
            Property Assessment Certification Guidelines
          </h2>
          <p className="mt-1.5 text-xs leading-relaxed text-gray-500 sm:text-sm">
            Request an official certification confirming the comprehensive assessment information and classification of a real property registered in Malaybalay City.
          </p>
          ...
          <div className="mt-5 rounded-2xl bg-zinc-50 p-4">
            ...
            {[
              ...
              "Proof of Ownership / Deed of Absolute Sale",
            ].map((r) => (
          ...
          <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-3.5 text-xs text-amber-800 flex items-start gap-2">
            ...
            <span>
              IMPORTANT: The AI performs automated consistency checks only. Final seal and certification approval are issued by the City Assessor.
            </span>
          </div>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2.5">
            ...
            {[
              { key: "normal", label: "Property Found (Pass)" },
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Target property assessment parameters</p>
            </div>
          </div>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="sm:col-span-2">
              <label className="mb-1.5 block text-xs font-bold text-gray-700">Property Owner (Declared Name)</label>
              <input
                type="text"
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. PROP-2026-00002"
                className="w-full rounded-xl border border-zinc-200 bg-zinc-50/50 px-3.5 py-2.5 text-xs text-gray-900 focus:border-red-600 focus:bg-white focus:outline-none focus:ring-1 focus:ring-red-600"
              />
            </div>
            <div>
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Attachments for assessment verification</p>
            </div>
          </div>
          <div className="space-y-3">
            <UploadBox label="Valid Government ID of Requester" file={idFile} onUpload={() => setIdFile({ name: "voters_id_santos.jpg", type: "image/jpeg", size: "1.1 MB", uploaded: true })} onRemove={() => setIdFile({ name: "", uploaded: false })} required />
            <UploadBox label="Existing Tax Declaration (if available)" file={tdFile} onUpload={() => setTdFile({ name: "tax_dec_santos.pdf", type: "application/pdf", size: "1.6 MB", uploaded: true })} onRemove={() => setTdFile({ name: "", uploaded: false })} />
            <UploadBox label="Proof of Ownership (Deed of Sale / TCT)" file={ownershipFile} onUpload={() => setOwnershipFile({ name: "deed_of_sale_santos.pdf", type: "application/pdf", size: "1.9 MB", uploaded: true })} onRemove={() => setOwnershipFile({ name: "", uploaded: false })} required />
            <UploadBox label="Authorization Letter (if representative)" file={authFile} onUpload={() => setAuthFile({ name: "auth_letter.pdf", type: "application/pdf", size: "0.5 MB", uploaded: true })} onRemove={() => setAuthFile({ name: "", uploaded: false })} />
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Consistency scan &amp; tax mapping check</p>
            </div>
          </div>
          ...
          {aiChecks.length === 0 && !aiScanning ? (
            <div className="flex flex-col items-center gap-3 py-8 text-center">
              ...
              <p className="text-sm font-bold text-gray-800">Ready to validate assessment request</p>
              <p className="max-w-xs text-xs text-gray-500">AI will scan for required documents and cross-check the property PIN and Tax Declaration numbers.</p>
              <button
                type="button"
                ...
                <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="rounded-full bg-emerald-600 px-2.5 py-0.5 text-[10px] font-extrabold text-white">
                      READY FOR ASSESSOR VERIFICATION
                    </span>
                  </div>
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          {isCheckingProperty && (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="text-xs sm:text-sm font-bold text-gray-700">Checking property assessment records…</p>
            </div>
          )}
          ...
          {!isCheckingProperty && matchedRecord && typeof matchedRecord === "object" && (
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 space-y-2 text-xs">
              <div className="flex items-center gap-2 font-extrabold text-emerald-800 mb-3">
                ...
                <span>Property Found &amp; Assessment Record Verified</span>
              </div>
          ...
          {!isCheckingProperty && matchedRecord === false && (
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs">
              ...
              <p className="text-amber-700">Property could not be matched automatically. Routed to Assessor staff for manual assessment archive check.</p>
            </div>
          )}
        </div>
    ...
    return (
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6 space-y-3">
          ...
          <SummaryRow label="Service" value="Property Assessment Certification" />
          ...
          <div className={`rounded-xl border px-3.5 py-2.5 text-xs font-bold ${matchedRecord && typeof matchedRecord === "object" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-50 text-amber-800"}`}>
            Property Check: {matchedRecord && typeof matchedRecord === "object" ? "VERIFIED (ACTIVE RECORD)" : "REQUIRES ASSESSOR REVIEW"}
          </div>
        </div>
        <NavButtons onBack={back} onNext={handleSubmit} nextLabel="Submit Assessment Certification Request" />
      </div>
    );
  }
  ...
  function StepTracking() {
    const trackSteps = [
      ...
      { label: "Property Verification", done: true },
      { label: "Assessor Processing", done: false, active: true },
    ...
    return (
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <h2 className="text-sm font-extrabold text-gray-900 mb-1">Assessment Certification Tracking</h2>
          ...
          <div className="mb-5 rounded-2xl border border-blue-200 bg-blue-50/60 p-4 text-xs">
            <p className="font-extrabold text-gray-900 text-xs sm:text-sm">Status: Assessor Processing</p>
            <p className="mt-1 text-gray-600">Your property assessment records have been matched. Assessor staff are preparing your official Property Assessment Certification.</p>
          </div>
    ...
    return (
      <div className="space-y-5 animate-fade-up">
        ...
        <div className="rounded-3xl border border-emerald-200 bg-emerald-50/40 p-5 text-center">
          ...
          <p className="text-base sm:text-lg font-extrabold text-gray-900">Property Assessment Certification Ready</p>
          <p className="text-xs text-gray-500 mt-1">Official certification issued by the City Assessor's Office.</p>
        ...
        <div className="relative overflow-hidden rounded-3xl border-2 border-zinc-200 bg-white p-5 sm:p-6 shadow-sm">
          ...
          <div className="relative z-10">
            <div className="text-center border-b border-zinc-200 pb-4 mb-4">
              ...
              <p className="mt-2 text-xs font-extrabold uppercase tracking-[0.2em] text-red-700">Property Assessment Certification</p>
            ...
            <div className="space-y-2 text-xs">
              ...
              <SummaryRow label="Declared Property Owner" value={property.ownerName} />
              <SummaryRow label="Property Location" value={property.propertyAddress} />
              <SummaryRow label="Property Classification" value={matchedRecord?.classification || "Residential (Class B)"} />
              <SummaryRow label="Assessed Value" value={matchedRecord?.assessedValue || "₱750,000.00"} />
              <SummaryRow label="Purpose" value={property.purpose} />
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-sm rounded-3xl bg-white p-6 shadow-2xl">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-sm font-extrabold text-gray-900">QR Assessment Verification</h3>
          ...
          <div className="space-y-2 text-xs border-t border-zinc-100 pt-4">
            <SummaryRow label="Document" value="Property Assessment Certification" />
      ...
      <div className="mt-4 flex items-center gap-3.5 rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm sm:p-5 animate-fade-up">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-xs text-gray-500">
            Official certification of real property assessment &amp; classification
          </p>
        </div>
      </div>
