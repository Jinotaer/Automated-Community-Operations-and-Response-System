// src/Citizen/SeniorCertificationFlow.jsx
...
export default function SeniorCertificationFlow({ office, cert }) {
  ...
  const [certInfo, setCertInfo] = useState({
    oscaRegistrationNumber: "OSCA-2026-00001",
    purpose: "Medical Assistance",
  });

  const [idFile, setIdFile] = useState({ name: "voters_id_santos.jpg", type: "image/jpeg", size: "1.1 MB", uploaded: true });
  const [oscaIdFile, setOscaIdFile] = useState({ name: "osca_card_maria.jpg", type: "image/jpeg", size: "0.8 MB", uploaded: true });
  const [residenceFile, setResidenceFile] = useState({ name: "brgy_cert_santos.pdf", type: "application/pdf", size: "0.7 MB", uploaded: true });
  ...
  const aiChecksDef = [
    ...
    { label: "OSCA Senior ID uploaded", passed: true },
    { label: "Proof of residence uploaded", passed: true },
  ...
  function runRegistryCheck() {
    ...
    setTimeout(() => {
      const found = testMode === "normal"
        ? findMockSeniorRecord({ fullName: applicant.fullName, oscaIdNumber: applicant.oscaIdNumber, registryNumber: certInfo.oscaRegistrationNumber })
        : null;
  ...
  function handleSubmit() {
    ...
    const certNo = `SC-CERT-2026-${Math.floor(100000 + Math.random() * 900000)}`;
    ...
    saveOSCARequest({
      ...
      registryVerification: matchedRecord
        ? { status: "MATCHED", registryNumber: certInfo.oscaRegistrationNumber, message: "Senior Citizen Record Found & Active in Masterlist." }
      ...
      documents: [
        ...
        { name: "OSCA ID", fileName: oscaIdFile.name, verified: oscaIdFile.uploaded },
      ...
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <h2 className="text-base font-extrabold text-gray-900 sm:text-lg">
            Senior Citizen Certification Guidelines
          </h2>
          <p className="mt-1.5 text-xs leading-relaxed text-gray-500 sm:text-sm">
            Request an official certification confirming your registered senior citizen status for social welfare, medical assistance, or legal requirements.
          </p>
          ...
          <div className="mt-5 grid grid-cols-2 gap-3 text-xs">
            ...
            <div className="rounded-2xl border border-zinc-100 bg-zinc-50/80 p-3.5">
              ...
              <p className="text-[11px] text-gray-400">Masterlist matched</p>
            </div>
          </div>
          ...
          <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-3.5 text-xs text-amber-800 flex items-start gap-2">
            ...
            <span>
              IMPORTANT: The AI performs automated consistency checks only. Final seal and certification approval are issued by OSCA.
            </span>
          </div>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2.5">
            ...
            {[
              { key: "normal", label: "Senior Record Found (Pass)" },
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Senior citizen identity details</p>
            </div>
          </div>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="mb-1.5 block text-xs font-bold text-gray-700">OSCA ID Number</label>
              <input
                type="text"
                value={applicant.oscaIdNumber}
                onChange={(e) => setApplicant((p) => ({ ...p, oscaIdNumber: e.target.value }))}
                placeholder="e.g. OSCA-2026-00025"
                className="w-full rounded-xl border border-zinc-200 bg-zinc-50/50 px-3.5 py-2.5 text-xs text-gray-900 focus:border-red-600 focus:bg-white focus:outline-none focus:ring-1 focus:ring-red-600"
              />
            </div>
            <div>
      ...
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <div>
              <h2 className="text-sm font-extrabold text-gray-900 sm:text-base">Certification Details</h2>
              <p className="text-xs text-gray-400">Specify reason and reference for certification</p>
            </div>
          </div>
          <div className="space-y-4">
            <div>
              <label className="mb-1.5 block text-xs font-bold text-gray-700">OSCA Registration / Masterlist Number</label>
              <input
                type="text"
                value={certInfo.oscaRegistrationNumber}
                onChange={(e) => setCertInfo((p) => ({ ...p, oscaRegistrationNumber: e.target.value }))}
                placeholder="e.g. OSCA-2026-00001"
                className="w-full rounded-xl border border-zinc-200 bg-zinc-50/50 px-3.5 py-2.5 text-xs text-gray-900 focus:border-red-600 focus:bg-white focus:outline-none focus:ring-1 focus:ring-red-600"
              />
            </div>
            <div>
              ...
              <select
                value={certInfo.purpose}
                onChange={(e) => setCertInfo((p) => ({ ...p, purpose: e.target.value }))}
                className="w-full rounded-xl border border-zinc-200 bg-zinc-50/50 px-3.5 py-2.5 text-xs text-gray-900 focus:border-red-600 focus:bg-white focus:outline-none focus:ring-1 focus:ring-red-600"
              >
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Attachments for status certification</p>
            </div>
          </div>
          <div className="space-y-3">
            <UploadBox label="Valid Government ID of Applicant" file={idFile} onUpload={() => setIdFile({ name: "voters_id_santos.jpg", type: "image/jpeg", size: "1.1 MB", uploaded: true })} onRemove={() => setIdFile({ name: "", uploaded: false })} required />
            <UploadBox label="OSCA Senior Citizen ID Card" file={oscaIdFile} onUpload={() => setOscaIdFile({ name: "osca_card_maria.jpg", type: "image/jpeg", size: "0.8 MB", uploaded: true })} onRemove={() => setOscaIdFile({ name: "", uploaded: false })} required />
            <UploadBox label="Proof of Residence (Barangay Certificate)" file={residenceFile} onUpload={() => setResidenceFile({ name: "brgy_cert_santos.pdf", type: "application/pdf", size: "0.7 MB", uploaded: true })} onRemove={() => setResidenceFile({ name: "", uploaded: false })} required />
            <UploadBox label="Supporting Document (Optional)" file={supportFile} onUpload={() => setSupportFile({ name: "doctor_referral.pdf", type: "application/pdf", size: "0.5 MB", uploaded: true })} onRemove={() => setSupportFile({ name: "", uploaded: false })} />
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Consistency scan &amp; masterlist check</p>
            </div>
          </div>
          ...
          {aiChecks.length === 0 && !aiScanning ? (
            <div className="flex flex-col items-center gap-3 py-8 text-center">
              ...
              <p className="text-sm font-bold text-gray-800">Ready to validate senior citizen certification</p>
              <p className="max-w-xs text-xs text-gray-500">AI will scan for required documents and cross-check the submitted OSCA ID number with active records.</p>
              <button
                type="button"
      ...
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="rounded-2xl border border-zinc-100 bg-zinc-50 p-4 text-xs mb-5 space-y-2">
            <p className="font-extrabold text-gray-700 uppercase tracking-wider text-[10px] mb-2">Submitted Certification Request</p>
            ...
            <SummaryRow label="Registration No." value={certInfo.oscaRegistrationNumber} />
          </div>
          ...
          {matchedRecord === null && !isCheckingRegistry && (
            ...
            >
              <ShieldCheck size={16} /> Verify Against OSCA Records
            </button>
          )}
          ...
          {isCheckingRegistry && (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="text-xs sm:text-sm font-bold text-gray-700">Checking senior citizen registration status…</p>
            </div>
          )}
          ...
          {!isCheckingRegistry && matchedRecord && typeof matchedRecord === "object" && (
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 space-y-2 text-xs">
              <div className="flex items-center gap-2 font-extrabold text-emerald-800 mb-3">
                ...
                <span>Senior Citizen Record Found &amp; Active</span>
              </div>
              <SummaryRow label="Registered Name" value={matchedRecord.fullName} />
              ...
              <SummaryRow label="Registration Status" value={matchedRecord.registrationStatus} />
          ...
          {!isCheckingRegistry && matchedRecord === false && (
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs">
              ...
              <p className="text-amber-700">Senior record could not be matched automatically. Forwarded to OSCA officer for manual masterlist check.</p>
            </div>
          )}
        </div>
    ...
    return (
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6 space-y-3">
          ...
          <SummaryRow label="Service" value="Senior Citizen Certification" />
          <SummaryRow label="Applicant Name" value={applicant.fullName} />
          <SummaryRow label="Date of Birth" value={applicant.dob} />
          <SummaryRow label="OSCA ID Number" value={applicant.oscaIdNumber} />
          <SummaryRow label="Registration No." value={certInfo.oscaRegistrationNumber} />
          <SummaryRow label="Purpose" value={certInfo.purpose} />
          <SummaryRow label="Registration Status" value="REGISTERED / ACTIVE" />
          ...
          <div className={`rounded-xl border px-3.5 py-2.5 text-xs font-bold ${matchedRecord && typeof matchedRecord === "object" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-50 text-amber-800"}`}>
            Registry Check: {matchedRecord && typeof matchedRecord === "object" ? "VERIFIED (ACTIVE RECORD)" : "REQUIRES OSCA REVIEW"}
          </div>
        </div>
        <NavButtons onBack={back} onNext={handleSubmit} nextLabel="Submit Senior Citizen Certification Request" />
      </div>
    );
  }
  ...
  function StepTracking() {
    const trackSteps = [
      ...
      { label: "OSCA Verification", done: false, active: true },
    ...
    return (
      <div className="space-y-5 animate-fade-up">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <h2 className="text-sm font-extrabold text-gray-900 mb-1">Senior Certification Tracking</h2>
          ...
          <div className="mb-5 rounded-2xl border border-blue-200 bg-blue-50/60 p-4 text-xs">
            <p className="font-extrabold text-gray-900 text-xs sm:text-sm">Status: OSCA Verification</p>
            <p className="mt-1 text-gray-600">Your certification request has been received and is queued for official signature and issuance by OSCA.</p>
          </div>
    ...
    return (
      <div className="space-y-5 animate-fade-up">
        ...
        <div className="rounded-3xl border border-emerald-200 bg-emerald-50/40 p-5 text-center">
          ...
          <p className="text-base sm:text-lg font-extrabold text-gray-900">Senior Citizen Certification Ready</p>
        ...
        <div className="relative overflow-hidden rounded-3xl border-2 border-zinc-200 bg-white p-5 sm:p-6 shadow-sm">
          ...
          <div className="relative z-10">
            <div className="text-center border-b border-zinc-200 pb-4 mb-4">
              ...
              <p className="mt-2 text-xs font-extrabold uppercase tracking-[0.2em] text-red-700">Senior Citizen Certification</p>
            ...
            <div className="space-y-2 text-xs">
              ...
              <SummaryRow label="OSCA ID Number" value={applicant.oscaIdNumber} />
              <SummaryRow label="Registration Number" value={certInfo.oscaRegistrationNumber} />
              <SummaryRow label="Address" value={applicant.address} />
              <SummaryRow label="Registration Status" value="ACTIVE / REGISTERED" />
              <SummaryRow label="Purpose" value={certInfo.purpose} />
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-sm rounded-3xl bg-white p-6 shadow-2xl">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-sm font-extrabold text-gray-900">QR Certification Verification</h3>
          ...
          <div className="space-y-2 text-xs border-t border-zinc-100 pt-4">
            ...
            <SummaryRow label="Document" value="Senior Citizen Certification" />
  ...
  const stepComponents = [
    <StepStart />, <StepApplicantInfo />, <StepCertInfo />, <StepUploadDocs />,
      ...
      <div className="mt-4 flex items-center gap-3.5 rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm sm:p-5 animate-fade-up">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-xs text-gray-500">
            Official certification of status · Social welfare &amp; assistance
          </p>
        </div>
      </div>
